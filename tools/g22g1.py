    ...
    for i in graph_g:
        for j in graph_g[i]:
            for m in graph_g[j]:
                ...
                elif (next_graph[i][m] == set([(0,2)]) or
                      next_graph[i][m] == set([(0,3)])):
                    next_graph[i][m] = set([(0,3)])
            for n in graph_g[i]:
                if j != n:
                    if not n in next_graph[j]:next_graph[j][n] = set([(0,2)])
                    elif (next_graph[j][n] == set([(0,1)]) or
                          next_graph[j][n] == set([(0,3)])):
                        next_graph[j][n] = set([(0,3)])
    return next_graph

# def find_next_graph(graph_g):
#     return increment_u(graph_g, graph_g)
# def compare(g_two_star,graph_g):
#     for n in g_two_star:
#         for h in g_two_star[n]:
#             try:
#                 if graph_g[n][h] != g_two_star[n][h]:
#                     return False
#             except KeyError:
#                     return False
#     return True
    ...
    for i in g_two_star:
        for j in g_two_star[i]:
            if j not in graph_g[i]:
                return False
            elif (g_two_star[i][j] != graph_g[i][j] and
                  graph_g[i][j] != set([(0,3)])):
                return False
    return True
...
def try_next_double_edge(g_de, g_one_star_de, stack_de):
    if stack_de != []:
        ...
        for k in g_one_star_de:
            boolean_first_edge_has_value = i in g_one_star_de[k]
            boolean_second_edge_has_value = j in g_one_star_de[k]
            g_one_star_de[k][i] = 1
            g_one_star_de[k][j] = 1
        ...
        print g_one_star_de
        print "G2 --------------------------"
        print find_next_graph(g_one_star_de)
        print "End-------------------------"
